...
def safe_request_json(url, params=None, headers=None, max_retries=5, base_delay=1, timeout=30):
    delay = base_delay
        ...
        try:
            ...
            if r.status_code == 200:
                try:
                    ...
                    print("Invalid JSON (empty or HTML). Retrying...")
            elif r.status_code == 429:
                print(f"Rate limited, waiting {delay}s...")
            elif r.status_code >= 500:
                print(f"Server error {r.status_code}, retrying...")
            else:
                print(f"Unexpected status {r.status_code}, giving up.")
                print(params)
                print(headers)
                return {}
